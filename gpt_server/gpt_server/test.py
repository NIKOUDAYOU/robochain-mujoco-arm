...
if __name__ == "__main__":
    env = GraspingEnv()
    env.reset()
    action = env.get_body_pos('green_block') - np.array([0.0, 0.0, 0.42])
    ...
    for t in range(int(100)):
        env.mj_data.actuator('0_gripper_l_finger_joint').ctrl[0] = -0.22
        env.mj_data.actuator('0_gripper_r_finger_joint').ctrl[0] = -0.2
        env.step(np.concatenate([action, np.array([1, 0, 0, 0])]))
